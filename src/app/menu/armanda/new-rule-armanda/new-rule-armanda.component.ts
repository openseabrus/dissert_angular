...
import { Poi } from 'src/app/database/poi';
...
import { DatabaseLinkService } from 'src/app/services/database-link.service';

@Component({
	selector: 'app-new-rule-armanda',
	templateUrl: './new-rule-armanda.component.html',
	styleUrls: ['./new-rule-armanda.component.css']
})
export class NewRuleArmandaComponent implements OnInit {
	...
	ngOnInit() {
		...
		this.databaseLinkService.getDatabase(true).subscribe(db => {
	...
	private submitRule() {
		...
		this.config.createRule(rule, true).subscribe(res => {
			this.router.navigate(['../../rules/armanda']);
		});
	}
